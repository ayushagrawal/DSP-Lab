...
Int16 impulse[FFT_LENGTH];

Int16 impulse_r_L[FFT_LENGTH];
Int16 impulse_i_L[FFT_LENGTH];

Int16 output_i_L[FFT_LENGTH];
Int16 output_r_L[FFT_LENGTH];
...
void main(void) 
{
	...
	{ 
		for(i = 0 ;i<overlap;i++)
			BufferL[i] = BufferL[FFT_LENGTH - i -1];
		 
		if (PingPongFlagInL && PingPongFlagInR) 
		{
			for (i = 0; i < PING_PONG_SIZE/2; i++)
			{
				BufferR[i+overlap] = DMA_InpL[i + PING_PONG_SIZE/2];
				BufferL[i+overlap] = DMA_InpR[i + PING_PONG_SIZE/2];
			}  		    
		} 
		else 
		{
			for (i = 0; i < PING_PONG_SIZE/2; i++)
			{
				BufferR[i+overlap] = DMA_InpL[i];
				BufferL[i+overlap] = DMA_InpR[i];
			}				
		}
		...
		for(i = 0; i<FFT_LENGTH;i++)
			impulse[i] = 0;
		for(i = 0; i<12;i++)
			impulse[i] = COEFFS[i]/500;
			
		do_fft(impulse,impulse_r_L,impulse_i_L,1);			
		...
		for(i=0; i<FFT_LENGTH;i++)
		{
			output_r_L[i] = realL[i]*impulse_r_L[i] - impulse_i_L[i]*imagL[i];
			output_i_L[i] = realL[i]*impulse_i_L[i] + impulse_r_L[i]*imagL[i];
		}
		...
		do_ifft(output_r_L, output_i_L, BufferL_out,1);
		/**************************HINT**********************************/
        ...
        if (PingPongFlagOutL && PingPongFlagOutR)
        {
			for (i = 0; i < PING_PONG_SIZE/2; i++)
			{
        		DMA_OutL[i + PING_PONG_SIZE/2] = 100*BufferL_out[i+overlap];
				DMA_OutR[i + PING_PONG_SIZE/2] = 100*BufferL_out[i+overlap];
        	}
        } 
        else 
        {
			for (i = 0; i < PING_PONG_SIZE/2; i++)
			{								  /*******HINT******/
        		DMA_OutL[i] = 100*BufferL_out[i+overlap]; //modify this code
				DMA_OutR[i] = 100*BufferL_out[i+overlap]; //its not correct
        	}
        }
	}
