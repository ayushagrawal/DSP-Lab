...
FIR_len	.set 11						;length of the impulse response ex: 12 length filter
...
; section 2: Start code for linear buffering from here
 	...
 	;BCLR SATD							; SATD=0 => no saturation is performed in case of overflow
 	 
	BSET SATD							; SATD=1 => no saturation is performed in case of overflow

	BSET M40							; M40=1 => 40bit mode operation for data computation....
										; Here we are dealing with accumulator (ACx) 

	;BCLR M40							; M40=0 => 32bit mode operation for data computation....
										; Here we are dealing with accumulator (ACx) 
...
; section 3: Convolution block start from here ....
	...
	;mov #0,AC0;	
	mov #0,AC0;	
						;Before performing the MAC operation always clear the contents of the accumulator.
