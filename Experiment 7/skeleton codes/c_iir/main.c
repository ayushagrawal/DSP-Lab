...
Int16 den_coeff[ORDER_IIR]={100,0,81};
...
{	
    ...
    /*****************************
     * Your code goes here */
	 acc = num_coeff[0]*x_IIR[i];
	 for(j = 1; j <= ORDER_IIR ; j++)
	 {
		 if(i>j)
			acc = acc + num_coeff[j]*x_IIR[i-j] - den_coeff[j]*y_IIR[i-j];
		else
			acc = acc + num_coeff[j]*x_IIR[ORDER_IIR+i-j] - den_coeff[j]*y_IIR[ORDER_IIR+i-j];
	 }
     /*****************************/
   // acc that we get here is multiplied by 100 of its original value
	y_IIR[i]=(Int16)(acc/32768);//Buffering the latest output
	return(y_IIR[i]);
}
...
int main( void )
{
    ...
    {    	
    	for(i=0;i<ORDER_IIR;i++)
    	{
    		...
    		output=500*direct_form_1(i,input);//Passing the value of 'i' also. One can also maintain a global counter instead.
