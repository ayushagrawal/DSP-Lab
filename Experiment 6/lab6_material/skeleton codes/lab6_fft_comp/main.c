...
*/
	...
	for(i=0;i<2;i++)
	{
		for(j=0;j<2;j++)
		{
			// Multiply selected values in stage 1 by the twiddle factors before calculating the butterfly.
				...
				/*Your Code Goes here*/
			complex_multiply(&stage_1_real[4*i+j+2], &stage_1_imag[4*i+j+2], &twiddle_real[2*j], &twiddle_imag[2*j], &temp_real, &temp_imag);
			  

			// Butterfly calculation

				/*Your Code Goes here*/	
			butterfly(&stage_1_real[4*i+j], &stage_1_imag[4*i+j], &temp_real, &temp_imag, &stage_2_real[4*i+j], &stage_2_imag[4*i+j], &stage_2_real[4*i+j+2], &stage_2_imag[4*i+j+2]);
		}
	}
	...
	for(i=0;i<4;i++)
	{
		// Multiply selected values in stage 2 by the twiddle factors appropriately before calculating the butterfly.
			...
			/*Your Code Goes here*/
		complex_multiply(&stage_2_real[i+4],&stage_2_imag[i+4],&twiddle_real[i],&twiddle_imag[i], &temp_real, &temp_imag);
		// Butterfly calculation (Note that y_real and y_imag themselves are POINTERS)

			/*Your Code Goes here*/
	    butterfly(&stage_2_real[i], &stage_2_imag[i], &temp_real, &temp_imag, &y_real[i], &y_imag[i], &y_real[i+4], &y_imag[i+4]);
	}
}

Int16 x[FFT_LENGTH]={1,0,2,0,3,0,4,0};
float x_sw[FFT_LENGTH] ={1,0,2,0,3,0,4,0};
float y_sw_real[FFT_LENGTH], y_sw_imag[FFT_LENGTH];
